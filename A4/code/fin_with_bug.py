...
def propagation_and_random_search_k(source_patches, target_patches,
    ...
    while alpha ** num_itr * w >= 1:
        ...
        num_itr += 1
    delta = np.array(delta).repeat(5).reshape(num_itr * k, 1)
    # print delta
    # adjust to shape of (k * num_itr)X2 matrix that easy for us to do operation.
    ...
    for i in loop_i:
        for j in loop_j:
            ...
            if propagation_enabled:
                ...
                if odd_iteration:
                    # Here I just found that left_org and up_org should swap...
                    left_org = {}  # offset of left patch of source img
                    up_org = {}  # offset of upper patch of source img
                ...
                # even loop (revers order)
                    ...
                    right_org = {}  # right patch of source img
                    down_org = {}  # lower patch of source img
                    right_dis = np.array([])  # distance of right patch of source img and it's corresponding target patch
            ...
            if random_enabled:
                ...
                # print np.array(f_coord_dictionary[i][j].keys())
                org_f = np.array([f_coord_dictionary[i][j].keys(), ] * num_itr).reshape((num_itr * k, 2))
                print org_f
                # if org_f.shape != delta.shape:
                #     print org_f, delta
                u_k = org_f + delta * R
                # print u_k.shape, len(f_coord_dictionary[i][j].keys())
                # It should be clamped by bound of B
                ...
                for l in range(len(distances)):
                    if distances[l] < min_dis:
                        # print distances[l], min_dis
                        new_offset = u_k[l]
                        if tuple(new_offset) not in f_coord_dictionary[i][j]:
                            # replace
                            f_coord_dictionary[i][j].pop(tuple(f_heap[i][j][0][2]), None)
                            f_coord_dictionary[i][j][tuple(new_offset)] = 0
                            heappushpop(f_heap[i][j], (-distances[l], _tiebreaker.next(), tuple(new_offset)))
                            min_dis = -f_heap[i][j][0][0]
    #############################################
...
def nlm(target, f_heap, h):
    ...
    #denoised = (cor_vj_image * np.array([wij, ] * 3)).sum(axis=0)
    #############################################
