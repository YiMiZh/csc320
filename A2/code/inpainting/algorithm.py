...
success, errorMessage = exampleBasedInpainting(self)
        ...
        if self._images['source'] is None or self._images['alpha'] is None:
            msg = 'Please check if you have source and alpha input'
            return success, msg
        if self._images['source'].shape[:2] != self._images['alpha'].shape[:2]:
            msg = 'Input images must have the same szie'
            return success, msg
        else:
            print self._images['source'].shape, self._images['alpha'].shape
            # check if source img is 3-channel and unit8
            if len(self._images['source'].shape) != 3 \
                    or self._images['source'].shape[2] != 3 \
                    or self._images['source'].dtype != 'uint8':
                msg = 'Source image should be 3-channel unit8 image'
                return success, msg
            # check if alpha img is 1-channel and unit8
            elif len(self._images['alpha'].shape) != 2 or self._images['alpha'].dtype != 'uint8':
                msg = 'Alpha image should be 1-channel unit8 image'
                return success, msg
        success = True
        #########################################
