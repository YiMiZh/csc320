...
def computeGradient(psiHatP=None, inpaintedImage=None, filledImage=None):
    ...
    # I use Sobel operator here to compute dx and dy (ksize=3 are more accurate)
    dx = cv.Sobel(g_patch, cv.CV_64F, 1, 0, ksize=3)
    dy = cv.Sobel(g_patch, cv.CV_64F, 0, 1, ksize=3)
    # get gradient
...
def computeNormal(psiHatP=None, filledImage=None, fillFront=None):
    ...
    if np.sum(fillFront == 255) == 1:
        ...
        dx = cv.Sobel(fill_p, cv.CV_64F, 1, 0, ksize=3)[psiHatP.radius()][psiHatP.radius()]
        dy = cv.Sobel(fill_p, cv.CV_64F, 0, 1, ksize=3)[psiHatP.radius()][psiHatP.radius()]
        # get unit vector
